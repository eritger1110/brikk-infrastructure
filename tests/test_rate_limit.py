"""
Test suite for Redis-based rate limiting functionality.

Tests the rate limiting service and integration with coordination endpoint including:
- Sliding window algorithm
- Per-org and per-key scoping
...
class TestRateLimitService:
    ...
    def mock_redis(self):
        ...
        mock_pipeline.execute.return_value = [0, 1, 5, True]  # [removed, added, count, expire]
    ...
    def test_rate_limit_exceeded(self, rate_limiter, mock_redis):
        ...
        mock_pipeline = MagicMock()
        mock_pipeline.execute.return_value = [0, 1, 85, True]  # 85 requests in window
        mock_redis.pipeline.return_value = mock_pipeline
    ...
    def test_redis_connection_failure(self, rate_limiter):
        ...
        rate_limiter.redis_client.pipeline.side_effect = Exception("Redis connection failed")
...
class TestRateLimitIntegration:
    ...
    def test_coordination_rate_limit_burst_behavior(self, mock_redis_factory, client, valid_envelope):
        ...
        with patch.dict(os.environ, {
            ...
            mock_pipeline = MagicMock()
            mock_pipeline.execute.return_value = [0, 1, 16, True]  # Above limit
            mock_redis.pipeline.return_value = mock_pipeline
            mock_redis.zrange.return_value = [('1640995140.123', 1640995140.123)]
    ...
    def test_coordination_rate_limit_scope_switching(self, mock_redis_factory, client, valid_envelope):
        ...
        with patch.dict(os.environ, {
            ...
            # Verify scope key generation (would be 'rlimit:anonymous' without auth)
            mock_pipeline.zremrangebyscore.assert_called()
            call_args = mock_pipeline.zremrangebyscore.call_args[0]
            assert 'rlimit:anonymous' in str(call_args[0])
    ...
    def test_coordination_rate_limit_redis_failure(self, mock_redis_factory, client, valid_envelope):
        ...
        with patch.dict(os.environ, {
            ...
            # Rate limit headers should still be present (with default values)
            assert 'X-RateLimit-Limit' in response.headers
            assert 'X-RateLimit-Remaining' in response.headers
            assert 'X-RateLimit-Reset' in response.headers
    ...
    def test_coordination_rate_limit_with_idempotency(self, client, valid_envelope):
        ...
        with patch.dict(os.environ, {
            ...
            with patch('src.services.rate_limit.get_rate_limiter') as mock_get_limiter:
                ...
                # Both rate limiting and idempotency should work
                assert 'X-RateLimit-Limit' in response.headers
                mock_limiter.check_rate_limit.assert_called_once()


class TestRateLimitGlobalInstance:
    ...
    def test_reset_rate_limiter(self):
        """Test resetting global rate limiter instance."""
        limiter1 = get_rate_limiter()
        reset_rate_limiter()
        limiter2 = get_rate_limiter()
