...
def test_discover_services(client: FlaskClient, auth_headers: dict):
    ...
    response = client.get(
        "/api/v1/discovery/discover?capability=data_processing",
        headers=auth_headers)
    assert response.status_code == 200
    assert len(response.json) == 1
    ...
    response = client.get(
        "/api/v1/discovery/discover?capability=non_existent",
        headers=auth_headers)
    assert response.status_code == 200
...
def test_service_heartbeat(client: FlaskClient, auth_headers: dict):
    ...
    response = client.post(
        f"/api/v1/discovery/services/{service_id}/heartbeat",
        headers=auth_headers)
    assert response.status_code == 200
...
def test_get_service_details(client: FlaskClient, auth_headers: dict):
    ...
    response = client.get(
        f"/api/v1/discovery/services/{service_id}",
        headers=auth_headers)
    assert response.status_code == 200
