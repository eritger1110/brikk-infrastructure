...
from src.models.agent import db, Purchase  # db + new Purchase model
...
# Helpers
# -------------------------------
ALNUM = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"  # avoid confusing chars
...
def _send_verification(email: str, vtoken: str, first_name: str = "", order_ref: str | None = None):
    ...
    html = f"""
      ...
      <p style="color:#9aa3b2">If you didn’t create this account, you can ignore this email.</p>
    """
...
def _jwt_login_response(user: User, extra_claims: dict | None = None):
...
def complete_signup():
    ...
    token is a short-lived JWT from the Netlify provision-link function (signed with PROVISION_SECRET).
    - Creates/updates User + password
    - Creates a Purchase row (with unique order_ref) if not already there
    - Sends verification email
    - Sets a JWT cookie so user can land in /app immediately
    """
    try:
        ...
        # Set verification token & expiry if your model supports it
        ...
        # Ensure a Purchase record exists so we have a professional order reference
        purchase = None
        if subscription_id:
        ...
        # Log in immediately (JWT cookie) and include order_ref as a claim
        return _jwt_login_response(user, {"order_ref": getattr(purchase, "order_ref", None)})
...
def verify_email():
    """
    Link target from email: /verify?token=... (via website proxy), which calls this.
    If valid, mark verified, set JWT cookie, redirect to APP_URL + APP_DASHBOARD_PATH.
    """
    ...
    try:
        ...
        dest = APP_URL.rstrip("/") + APP_DASHBOARD_PATH
        resp = redirect(dest)
        identity = str(getattr(user, "id", getattr(user, "email", "")))
        token_jwt = create_access_token(identity=identity, additional_claims={"email": getattr(user, "email", None)})
...
# -------------------------------
@security_bp.get("/me")
@security_bp.get("/auth/me")  # alias for convenience
...
def me():
    ...
    return jsonify({
        ...
        "order_ref": claims.get("order_ref"),
