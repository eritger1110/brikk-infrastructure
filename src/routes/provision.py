...
def signup_from_stripe():
    ...
    try:
        ...
        email = (session.customer_details.email
                 ...
                 else (getattr(session.customer, 'email', None) if isinstance(session.customer, object) else None))
        if not email:
            return jsonify(success=False, error='No email on Stripe session'), 400
        ...
        if first:
            ...
            if hasattr(user, 'first_name'): user.first_name = first
            if hasattr(user, 'last_name'):  user.last_name  = last
        ...
        # Skipped here to keep the example tight; you can reuse the email code we added earlier.

        # Issue a login cookie so they land signed in
        access_token = create_access_token(identity=str(user.id), expires_delta=timedelta(days=7))
        resp = jsonify(success=True, next=APP_DASHBOARD_PATH)
        set_access_cookies(resp, access_token, domain=COOKIE_DOMAIN if COOKIE_DOMAIN else None, samesite='None', secure=True)
        return resp
