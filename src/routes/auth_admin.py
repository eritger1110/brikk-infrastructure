...
def require_admin_token(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not HMACSecurityService.constant_time_compare(provided_token, admin_token):
            return jsonify({
...
def create_organization():
    ...
    try:
        ...
        current_app.logger.info(f"Created organization: {org.slug} (ID: {org.id})")
...
def list_organizations():
    ...
    try:
        ...
        return jsonify([OrganizationResponse(**org.to_dict()).dict() for org in organizations])
    except Exception as e:
...
def create_agent(slug):
    ...
    try:
        ...
        current_app.logger.info(f"Created agent: {agent.agent_id} for org {org.slug}")
...
def list_agents(slug):
    ...
    try:
        ...
        return jsonify([AgentResponse(**agent.to_dict()).dict() for agent in agents])
    except Exception as e:
...
def create_api_key():
    ...
    try:
        ...
        agent = None
        if data.agent_id:
            agent = Agent.query.filter_by(id=data.agent_id, organization_id=org.id).first()
            if not agent:
                return jsonify({
        ...
        current_app.logger.info(f"Created API key: {api_key.key_prefix}*** for org {org.slug}")
...
def disable_api_key():
    ...
    try:
        ...
        current_app.logger.info(f"Disabled API key: {api_key.key_prefix}*** (reason: {data.reason})")
...
def list_api_keys():
    ...
    try:
        ...
        return jsonify([ApiKeyResponse(**key.to_dict()).dict() for key in api_keys])
