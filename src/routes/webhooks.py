...
def create_webhook():
    ...
    try:
        ...
        if not data or not all(k in data for k in ["url", "secret", "events"]):
            return jsonify(
                {"error": "Missing required fields: url, secret, events"}), 400
        ...
        return jsonify({"id": webhook.id, "url": webhook.url,
                       "events": webhook.events}), 201
...
def get_webhooks():
    ...
    try:
        ...
        webhooks = webhook_service.get_webhooks_for_organization(
            organization_id)
...
def update_webhook(webhook_id: int):
    ...
    try:
        ...
        return jsonify({"id": updated_webhook.id,
                       "message": "Webhook updated successfully"}), 200
...
@webhooks_bp.route("/api/v1/webhooks/events/<int:event_id>/retry",
                   methods=["POST"])
@jwt_required()
def retry_webhook_event(event_id: int):
    ...
    try:
        ...
        if success:
            return jsonify(
                {"message": f"Webhook event {event_id} retry initiated"}), 202
        else:
            return jsonify(
                {"error": f"Failed to retry webhook event {event_id}"}), 500
...
def test_webhook():
    ...
    try:
        ...
        if not data or "webhook_id" not in data:
            return jsonify(
                {"error": "Missing required field: webhook_id"}), 400
        ...
        test_payload = {
            "message": "This is a test event from the Brikk platform."}
        webhook_service.trigger_event(
            "test.event", test_payload, organization_id)

        return jsonify(
            {"message": f"Test event sent to webhook {webhook_id}"}), 202
