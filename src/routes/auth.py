...
# ---------- tiny diagnostics so you can prove routing/cors quickly ----------
@auth_bp.get("/_ping")
def _ping():
    return jsonify({
        "success": True,
...
@auth_bp.get("/_routes")
def _routes():
    # list the routes that belong to this blueprint so you can see what is mounted
    rules = []
...
def complete_signup():
    ...
    # simple shared-secret, not a JWT
