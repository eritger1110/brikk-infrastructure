...
@discovery_bp.route("/api/v1/discovery/services/<int:service_id>",
                    methods=["GET"])
@jwt_required()
...
@discovery_bp.route("/api/v1/discovery/services/<int:service_id>/heartbeat",
                    methods=["POST"])
@jwt_required()
...
def cleanup_expired_services():
    ...
    try:
        ...
        return jsonify(
            {"message": f"Removed {removed_count} expired services"}), 200
