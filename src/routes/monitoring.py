...
def get_analytics():
    ...
    try:
        ...
        analytics_data = monitoring_service.get_performance_analytics(time_range)
...
def get_agent_monitoring():
    ...
    try:
        ...
        if agent_id:
            # In a real implementation, this would filter for specific agent data
        ...
        return jsonify({'error': 'Failed to retrieve agent monitoring data'}), 500
...
def get_security_monitoring():
    ...
    try:
        ...
        failed_auth_rate = security_metrics.get('failed_auth_attempts', 0) / max(security_metrics.get('total_auth_events', 1), 1)
        ...
        return jsonify({'error': 'Failed to retrieve security monitoring data'}), 500
...
def get_performance_monitoring():
    ...
    try:
        ...
        agent_metrics = monitoring_service.get_agent_metrics(5)  # 5-minute window
        
        # Calculate performance indicators
        performance_data = {
            'response_time': {
                'current': agent_metrics.get('avg_response_time_ms', 0),
                'threshold': 2000,
                'status': 'good' if agent_metrics.get('avg_response_time_ms', 0) < 1000 else 'warning'
            },
            'throughput': {
                'coordinations_per_minute': agent_metrics.get('recent_coordinations', 0) / 5,
                'status': 'good'
            },
            'success_rate': {
                'current': agent_metrics.get('coordination_success_rate', 0),
                'threshold': 95,
                'status': 'good' if agent_metrics.get('coordination_success_rate', 0) >= 95 else 'warning'
            },
            'agent_utilization': {
                'current': agent_metrics.get('agent_utilization', 0),
                'optimal_range': [60, 80],
                'status': 'good'
            }
        }
...
def get_dashboard_data():
    ...
    try:
        ...
        kpis = {
            ...
            'total_coordinations_today': agent_metrics.get('recent_coordinations', 0) * 24,  # Extrapolated
        ...
        dashboard_data = {
            ...
            'security_summary': {
                'threat_level': 'low',
                'failed_auth_attempts': security_metrics.get('failed_auth_attempts', 0),
                'rate_limit_hits': security_metrics.get('rate_limit_hits', 0)
            },
            'active_alerts': active_alerts,
