...
class WorkflowService:
    ...
    def get_workflows_by_organization(
            self, organization_id: int) -> list[Workflow]:
        return self.db.query(Workflow).filter(
            Workflow.organization_id == organization_id).all()
    ...
    def execute_workflow(
        ...
        workflow_execution = WorkflowExecution(
            workflow_id=workflow_id, context=context, status="running")
        ...
        # to execute the workflow steps. For now, we'll just mark it as
        # running.
