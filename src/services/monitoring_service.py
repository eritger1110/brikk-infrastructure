...
class MonitoringService:
    ...
    def get_agent_metrics(self, time_range_minutes: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_response_time = self._calculate_avg_response_time(time_range_minutes)
            ...
            self.record_metric('coordinations.success_rate', success_rate / 100)
            self.record_metric('coordinations.avg_response_time', avg_response_time)
        ...
        try:
            ...
            for component, status in health.items():
                if isinstance(status, dict) and 'healthy' in status:
                    self.record_metric(f'system.{component}.healthy', 1 if status['healthy'] else 0)
    ...
    def get_security_metrics(self, time_range_minutes: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            security_events = db.session.query(
                ...
                AuditLog.created_at >= cutoff_time,
                AuditLog.event_type.in_(['auth_success', 'auth_failure', 'rate_limit_hit', 'suspicious_activity'])
        ...
        try:
            ...
            for alert_rule in self.alert_rules:
                ...
                if is_triggered and not alert_rule.is_active:
                    ...
                    # Log alert
                    logger.warning(f"Alert triggered: {alert_rule.name} - {alert_rule.description}")
    ...
    def get_performance_analytics(self, time_range_hours: int = 24) -> Dict[str, Any]:
        """Get detailed performance analytics"""
        try:
            cutoff_time = datetime.now(timezone.utc) - timedelta(hours=time_range_hours)
            
            # Coordination patterns
            coordination_patterns = self._analyze_coordination_patterns(cutoff_time)
    ...
    def _calculate_avg_response_time(self, time_range_minutes: int) -> float:
        ...
        try:
            ...
            active_agents = db.session.query(Agent).filter(Agent.status == 'active').count()
    ...
    def _check_api_health(self) -> Dict[str, Any]:
        ...
        # Simplified health check - in production, this would test actual endpoints
        return {
            'healthy': True,
            'endpoints_checked': 5,
            'endpoints_healthy': 5,
    ...
    def _evaluate_alert_condition(self, alert: Alert, agent_metrics: Dict, 
                                 system_health: Dict, security_metrics: Dict) -> bool:
        """Evaluate if an alert condition is met"""
        try:
            if alert.id == "high_error_rate":
                success_rate = agent_metrics.get('coordination_success_rate', 100)
                ...
                if total_agents > 0:
                    inactive_ratio = (total_agents - active_agents) / total_agents
            ...
            logger.error(f"Failed to evaluate alert condition for {alert.id}: {e}")
            return False
    
    def _analyze_coordination_patterns(self, cutoff_time: datetime) -> Dict[str, Any]:
    ...
    def _analyze_agent_performance(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            agent_activity = db.session.query(
                Agent.id,
                Agent.name,
                func.count(Coordination.id).label('coordination_count')
            ).outerjoin(
                Coordination, 
                (Agent.id == Coordination.sender_agent_id) | (Agent.id == Coordination.recipient_agent_id)
            ).filter(
                Coordination.created_at >= cutoff_time
            ...
            return {
                'agent_activity': [
                    ...
                    for agent_id, name, count in agent_activity
                ],
                'most_active_agent': max(agent_activity, key=lambda x: x[2])[1] if agent_activity else None
            }
        except Exception as e:
