...
class DiscoveryService:
    ...
    def __init__(self, db_session: Session):
        self.db = db_session
        self.service_ttl_minutes = int(os.getenv("AGENT_SERVICE_TTL_MINUTES", "60"))
        
    def register_service(self, agent_id: int, service_name: str, service_url: str, 
        ...
        try:
            ...
            if service:
                ...
                service.expires_at = datetime.now(timezone.utc) + timedelta(minutes=self.service_ttl_minutes)
                ...
                service = AgentService(
                    ...
                    name=service_name,
                    url=service_url,
                    expires_at=datetime.now(timezone.utc) + timedelta(minutes=self.service_ttl_minutes)
                )
                self.db.add(service)
            ...
            self.db.commit()
            logger.info(f"Registered service '{service_name}' for agent {agent_id}")
            return service
    ...
    def _update_service_capabilities(self, service: AgentService, capabilities: List[str]) -> None:
        """Update the capabilities for a given service"""
        # Remove old capabilities
        self.db.query(AgentCapability).filter_by(service_id=service.id).delete()
    ...
    def discover_services(self, capability: Optional[str] = None, 
        ...
        try:
            ...
            if capability:
                query = query.join(AgentCapability).filter(AgentCapability.name == capability)
    ...
    def _format_service_response(self, service: AgentService) -> Dict[str, Any]:
