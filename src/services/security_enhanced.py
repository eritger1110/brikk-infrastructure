...
class HMACSecurityService:
    ...
    @classmethod
    def validate_request_headers(cls, headers: Dict[str, str]) -> Tuple[bool, Optional[str], Dict[str, str]]:
        """
        Validate required authentication headers are present.
        
        Returns (is_valid, error_message, extracted_headers)
        """
        required_headers = ['X-Brikk-Key', 'X-Brikk-Timestamp', 'X-Brikk-Signature']
        extracted = {}
    ...
    @staticmethod
    def create_error_response(code: str, message: str, request_id: Optional[str] = None) -> Dict[str, Any]:
