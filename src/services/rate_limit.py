...
class RateLimitService:
    ...
    def __init__(self, redis_client: Optional[redis.Redis] = None):
        ...
        self.scope = self._get_env('BRIKK_RLIMIT_SCOPE', 'org')  # 'org' or 'key'
        
        # Validate configuration
        if self.scope not in ['org', 'key']:
            logger.warning(f"Invalid BRIKK_RLIMIT_SCOPE '{self.scope}', defaulting to 'org'")
            self.scope = 'org'
    ...
    def get_scope_key(self, organization_id: Optional[str], api_key_id: Optional[str]) -> str:
        """
    ...
    def _check_rate_limit_redis(self, scope_key: str) -> RateLimitResult:
        ...
        if not allowed:
            ...
            try:
                oldest_scores = self.redis_client.zrange(scope_key, 0, 0, withscores=True)
                if oldest_scores:
                    oldest_time = oldest_scores[0][1]
                    retry_after = max(1, int(oldest_time + self.window_size - now))
            except Exception:
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'status': 'healthy' if (not self.enabled or redis_healthy) else 'degraded',
