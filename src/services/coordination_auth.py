...
class CoordinationAuthService:
    ...
    @staticmethod
    def create_error_response(code: str,
                              message: str,
                              status_code: int = 400,
                              details: Optional[list] = None,
                              request_id: Optional[str] = None) -> Dict[str,
                                                                        Any]:
    ...
    def _authenticate_jwt_session(
            self, request_id: str) -> Tuple[bool, Optional[Dict[str, Any]], Optional[int]]:
        """
    ...
    def _authenticate_hmac(self,
        ...
        try:
            ...
            api_key_header = request.headers.get(
                'X-Brikk-Key') or request.headers.get('Authorization', '').replace('Bearer ', '')
            ...
            try:
                timestamp_valid = self.hmac_service.verify_timestamp_drift(
                    timestamp, max_drift_seconds=300)
    ...
    def check_idempotency(self,
        ...
        try:
            ...
            should_process, response, status = self.idempotency_service.process_request_idempotency(
                key_id=key_id, body_hash=body_hash, custom_idempotency_key=custom_idem_key)
    ...
    def validate_feature_flags(self) -> Dict[str, bool]:
        ...
        return {
            ...
            "idempotency_enabled": self.get_feature_flag(
                "BRIKK_IDEM_ENABLED",
                "true"),
            "rate_limiting_enabled": self.get_feature_flag(
                "BRIKK_RATE_LIMIT_ENABLED",
                "true"),
