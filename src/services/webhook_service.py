...
class WebhookService:
    ...
    def create_webhook(self, organization_id: int, url: str, secret: str,
        ...
        try:
            ...
            self.db.commit()
            logger.info(
                f"Created webhook {webhook.id} for organization {organization_id}")
            return webhook
    ...
    def get_webhooks_for_organization(
            self, organization_id: int) -> List[Webhook]:
        """Get all webhooks for an organization"""
        return self.db.query(Webhook).filter_by(
    ...
    def trigger_event(self,
        ...
        try:
            ...
            if not subscribed_webhooks:
                logger.debug(
                    f"No active webhooks for event {event_type} in org {organization_id}")
                return
    ...
    def _create_and_send_webhook_event(
            self,
            webhook: Webhook,
        ...
        try:
            ...
            self.db.rollback()
            logger.error(
                f"Failed to create webhook event for webhook {webhook.id}: {e}")
    ...
    def send_webhook_event(self, event_id: int) -> bool:
        ...
        try:
            ...
            if not webhook or not webhook.is_active:
                logger.warning(
                    f"Webhook {webhook.id} is inactive or deleted, skipping event {event_id}")
            ...
            if 200 <= response.status_code < 300:
                ...
                self.db.commit()
                logger.info(
                    f"Webhook event {event_id} sent successfully to {webhook.url}")
                return True
            else:
    ...
    def retry_failed_events(self) -> None:
        ...
        try:
            ...
            for event in failed_events:
                logger.info(
                    f"Retrying webhook event {event.id} (attempt {event.retry_count + 1})")
                self.send_webhook_event(event.id)
    ...
    def _handle_failed_delivery(
        ...
        try:
            ...
            logger.warning(
                f"Webhook event {event.id} failed to deliver to {event.webhook.url} with status {response.status_code}")

            # Schedule retry if applicable
            if event.retry_count < self.max_retries:
                # In a real implementation, this would use a background job
                # queue (e.g., Celery)
                logger.info(f"Scheduling retry for webhook event {event.id}")
            else:
                logger.error(
                    f"Webhook event {event.id} has reached max retries and will not be sent again")

        except Exception as e:
            self.db.rollback()
            logger.error(
                f"Failed to handle failed delivery for event {event.id}: {e}")
