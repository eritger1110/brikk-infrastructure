...
class AuthMiddleware:
    ...
    def is_feature_enabled(self, feature_flag: str, default: bool = False) -> bool:
        """Check if feature flag is enabled."""
        return os.environ.get(feature_flag, str(default).lower()).lower() == 'true'
    
    def authenticate_request(self) -> Tuple[bool, Optional[Dict[str, Any]], Optional[int]]:
        """
        ...
        try:
            ...
            if not self.is_feature_enabled('BRIKK_FEATURE_PER_ORG_KEYS', False):
            ...
            if not api_key.organization.is_within_limits():
                return False, self.hmac_service.create_error_response(
                    'rate_limit_exceeded', 'Organization monthly request limit exceeded'
                ), 429
    ...
    def check_idempotency(self) -> Tuple[bool, Optional[Dict[str, Any]], Optional[int]]:
        """
    ...
    def cache_response(self, response_data: Dict[str, Any], status_code: int = 200):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if isinstance(result, tuple) and len(result) == 2:
            ...
            try:
                ...
                if response_data:
                    auth_middleware.cache_response(response_data, result.status_code)
            except:
