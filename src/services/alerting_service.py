...
class AlertingService:
    ...
    def _load_notification_templates(
            self) -> Dict[NotificationChannel, NotificationTemplate]:
...
{details}
    ...
    def process_alert(self, alert_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not applicable_rules:
                logger.debug(
                    f"No applicable notification rules for alert {alert_id}")
                return True

            # Send notifications for each applicable rule
            success = True
            for rule in applicable_rules:
                rule_success = self._send_notifications_for_rule(
                    alert_data, rule)
            ...
            logger.error(
                f"Failed to process alert {alert_data.get('id', 'unknown')}: {e}")
            return False
    ...
    def send_test_notification(
        ...
        try:
            ...
            if channel == NotificationChannel.EMAIL:
                ...
                logger.warning(
                    f"Test notification not implemented for channel: {channel}")
                return False
    ...
    def get_notification_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for channel in NotificationChannel:
                status['channels'][channel.value] = self._check_channel_configuration(
                    channel)
    ...
    def _send_notifications_for_rule(
            self, alert_data: Dict[str, Any], rule: NotificationRule) -> bool:
        ...
        for channel in rule.channels:
            try:
                if channel == NotificationChannel.EMAIL:
                    channel_success = self._send_email_notification(
                        alert_data, rule.recipients)
                elif channel == NotificationChannel.SLACK:
                    channel_success = self._send_slack_notification(
                        alert_data, rule.recipients)
                elif channel == NotificationChannel.WEBHOOK:
                    channel_success = self._send_webhook_notification(
                        alert_data, rule.recipients)
                else:
                    logger.warning(
                        f"Notification channel not implemented: {channel}")
    ...
    def _send_email_notification(
            self, alert_data: Dict[str, Any], recipients: List[str]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Email notification sent for alert {alert_data.get('id')} to {len(recipients)} recipients")
            return True
    ...
    def _send_slack_notification(
        ...
        try:
            ...
            logger.info(
                f"Slack notification sent for alert {alert_data.get('id')}")
            return True
    ...
    def _send_webhook_notification(
            self, alert_data: Dict[str, Any], webhook_urls: List[str]) -> bool:
    ...
    def _check_channel_configuration(
            self, channel: NotificationChannel) -> Dict[str, Any]:
